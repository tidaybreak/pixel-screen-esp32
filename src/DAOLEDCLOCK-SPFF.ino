...
void myconfig()
{
  ...
  temp_mod = e_int.val;
  if (temp_mod >= 0 && temp_mod < 32768)
    temp_mod = temp_mod;
  if (temp_mod >= 32768)
    temp_mod = temp_mod - 65536;
  e_int.val_b[0] = EEPROM.read(2);
  e_int.val_b[1] = EEPROM.read(3);
  hum_mod = e_int.val;
  if (hum_mod >= 0 && hum_mod < 32768)
    hum_mod = hum_mod;
  if (hum_mod >= 32768)
    hum_mod = hum_mod - 65536;
  ...
  Serial.println(String("config city:") + String(city));
  Serial.println(String("config zx_key:") + String(zx_key));
}
void saveconfig()
{
  EEPROM.begin(1024);
