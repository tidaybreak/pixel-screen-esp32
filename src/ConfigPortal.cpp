...
static const char *serverIndex =
    ...
    "function submit4() {      $.ajax({ type: 'GET', url: '/draw', data: {text: $('#text').val(), args: $('#args').val()}, dataType: 'html', success: function(result) {   }     });   };"
    "</script></head>"
...
void handleRoute(){
  ...
  wm.server->send(200, "text/html; charset=utf-8", serverIndex);
}


void handleDraw() // 回调函数
{
  String content = wm.server->arg("text");
  String args = wm.server->arg("args");
  ...
  //test(content, clear, x, y, color.c_str(), fsize);

  wm.server->send(200, "text/plain", "alert('success!')");
}

void bindServerCallback(){
  //wm.server->on("/custom",handleRoute); // this is now crashing esp32 for some reason
  //wm.server->on("/draw",handleDraw); // you can override wm!
...
void setup_ConfigPortal() {
  ...
  WiFiManagerParameter custom_html("<p style=\"color:pink;font-weight:Bold;\">后台</p>"); // only custom html
  ...
  const char* menuhtml = "<form action='/custom' method='get'><button>自定义</button></form><br/>\n";
...
*/
  ...
  if(!wm.autoConnect("Pixel","12345678")) {
    //clearOLED();
    //text("failed to connect and hit timeout");
  }
  else if(TEST_CP) {
    // start configportal always
    delay(1000);
    text("TEST_CP ENABLED...");
    wm.setConfigPortalTimeout(TESP_CP_TIMEOUT);
    wm.startConfigPortal("Pixel","12345678");
  }
  else {
